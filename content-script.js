...
let fetchDate = (el) => {
    date = el.querySelector("div.slide > div.eventzone > div > div.data > div > div.subtitle > span > span:nth-child(2)");
...
Array.prototype.forEach.call(els, function(el) {
    ...
    let location = el.querySelector("div.slide > div.eventzone > div > div.data > div > div.item.location > span.label");
    if (location) {
        location = location.innerText.trim();
        location = urlCompliance(location);
    }
    let date = fetchDate(el);
    ...
    let rButton = el.querySelector(`div.slide > div.eventzone > div > div.data > div > div.buttons > span > span.count`);
    if (rButton)
